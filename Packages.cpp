...
void __fastcall TPackageForm::btnSaveClick(TObject *Sender)
{
	...
	if(myFile.is_open()){
		...
		AnsiString pkgNo = txtPkgCode -> Text;
		...
		txtItemName -> Text = "";
		txtQty -> Text = "";
        listPkgDetails -> Items -> Clear();
        total =0;
	}
}
//---------------------------------------------------------------------------

void __fastcall TPackageForm::btnViewAllClick(TObject *Sender)
{
    listAllPkg -> Items -> Clear();
    fstream myFile;
	myFile.open("Packages.txt",ios::in);
	String pkgNo = "";
	String pkgName = "";
	String total = "";
	String comma = ",";
	...
	if(myFile.is_open()){
		...
		while(getline(myFile,line)){
			std::vector<std::string> parsedLine = parseCommaDelimitedString(line);
			pkgNo = parsedLine.at(0);
			pkgName = parsedLine.at(1);
			total = parsedLine.at(2);

			list = pkgNo+comma+pkgName+comma+total;
			listAllPkg -> Items -> Add(list);
		}
